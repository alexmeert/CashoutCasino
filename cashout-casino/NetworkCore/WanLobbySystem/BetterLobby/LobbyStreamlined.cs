...
public partial class LobbyStreamlined : Node
	...
	[Export]
	private int portOffset = 100; // Game servers start at PortMinimum+100 (9100) to avoid lobby port 9000
	...
	public override void _Process(double delta)
		...
		// Hide the WAN lobby panel once connected to a game server (or if we ARE the master).
		// GameMaster handles showing/hiding its own UI — don't touch GameRoot children here.
		bool inGame = GenericCore.Instance.IsGenericCoreConnected;
		((Control)GetChild(0)).Visible = !inGame && !IsWanLobbyServer;
	}
	...
	private void UpdateVBoxChildren(VBoxContainer vbox)
	{
		foreach (Node c in vbox.GetChildren())
		{
			...
			{
				var btn = child.GetNodeOrNull<Button>("Button");
				child.Visible = btn != null && btn.Visible;
			}
		}
		vbox.QueueSort();
	}
